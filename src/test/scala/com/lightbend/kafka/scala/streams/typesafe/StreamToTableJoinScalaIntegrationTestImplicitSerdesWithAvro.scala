...
import com.lightbend.kafka.scala.streams.{StatelessScalaSerde, StreamToTableJoinTestData}
...
object StreamToTableJoinScalaIntegrationTestImplicitSerdesWithAvro
  ...
  /*
  An alternative definition which would make *all* classes (that can produce
  instances for SchemaFor, FromRecord and ToRecord) to be automatically
  converted to Avro via the AvroSerde conversion. This may not always be
  desirable, because once you have an implicit resolution for a type, if you
  add another one you may find yourself with conflicting implicit types.

  implicit def avroSerdeGenerator[T >: Null : SchemaFor : FromRecord : ToRecord]: Serde[T] = {
    new AvroSerde[T]
  }
  */
  ...
  test("should count clicks per region") { server =>
    ...
    val userClicksStream: TSKStream[String, UserClicks] = TSKStream(userClicksTopic)
    ...
    val clicksPerRegion: TSKTable[String, Long] =
      userClicksStream
        ...
        .leftJoin(
        userRegionsTable,
        (clicks: UserClicks, region: String) =>
          (if (region == null) "UNKNOWN" else region, clicks.clicks))
