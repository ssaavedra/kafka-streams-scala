...
import org.apache.kafka.streams.kstream.{Materialized, TimeWindowedKStream, Windowed}

class TSTimeWindowedKStream[K, V](
    override protected[typesafe] val unsafe: TimeWindowedKStream[K, V]
) extends AnyVal
    with TSKType[TimeWindowedKStream, K, V] {
  ...
  def count(
      implicit materialized: Materialized[K, java.lang.Long, wsb]
