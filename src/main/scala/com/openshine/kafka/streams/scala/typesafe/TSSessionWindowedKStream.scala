...
import org.apache.kafka.streams.kstream.{Materialized, SessionWindowedKStream, Windowed}

class TSSessionWindowedKStream[K, V](
    override protected[typesafe] val unsafe: SessionWindowedKStream[K, V]
) extends AnyVal
    with TSKType[SessionWindowedKStream, K, V] {
  ...
  def count(
      implicit materialized: Materialized[K, java.lang.Long, ssb]
